...
- Voice AI (`/api/voice/process-request`) enriched with real-time data: ESPN sports scores (NBA/NFL/MLB/NHL/college), OpenWeatherMap weather, NewsData.io news, Wikipedia knowledge, and Bible verses (KJV). Also fetches user interests from the adaptive learning server (`/api/onboarding/profile/:userId`). All external data is sanitized via `sanitizeExternalText()` and wrapped with inert-data boundaries in the system prompt. AI assistant names: female=**Ida**, male=**Clay** (configurable in PreferencesContext).
