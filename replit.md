...
- Daily Quotes: The home screen displays an inspirational quote (italicized) above the voice orb before a conversation starts. When the user taps the orb, the quote animates off-screen (slides right with fade). Quotes rotate daily from a curated collection in `constants/dailyQuotes.ts`. Users can disable this feature via a toggle in Settings. The preference is stored as `daily_quotes_enabled` in the users table.
