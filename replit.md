...
- Voice AI system: Dynamically injects user's active daily reminders into conversation context and is enriched with real-time data from external APIs (sports, weather, news, knowledge, bible verses) and user interests. AI assistant names are configurable (Ida/Clay). Weather city parsing uses multi-pattern extraction with US state abbreviation mapping and fallback handling. Sports detection recognizes all major pro team names (NBA/NFL/MLB/NHL) with ambiguous name resolution (e.g., "panthers" defaults to NFL). Team-specific filtering narrows ESPN results to the requested team. All real-time API calls use `fetchWithRetry()` with fallback context messages.
